...
from utils.arg_utils import update_dict_config_with_dot_notation_override_args
...
class ExperimentsPlan:
    ...
    @staticmethod
    def load_from(plan_config_path: str, overrides_dict: dict = None) -> ExperimentsPlan:
        with open(plan_config_path) as f:
            raw_plan_config = json.load(f)
            return ExperimentsPlan(raw_plan_config, overrides_dict)

    def __init__(self, raw_plan_config: dict, overrides_dict: dict = None):
    ...
    def __create_experiment_configurations_for_base_config(self, base_config: dict, options: dict) -> List[dict]:
        if len(options) == 0:
            ...
            if self.overrides_dict:
                update_dict_config_with_dot_notation_override_args(config, self.overrides_dict)
