...
class MetricsPlotter(Callback):
    ...
    def on_fit_initialization(self, trainer):
        if self.accelerator.is_main_process and self.create_dir and not os.path.exists(self.plots_dir):
            os.makedirs(self.plots_dir, exist_ok=True)

    def on_epoch_end(self, trainer):
        if self.accelerator.is_main_process and (trainer.epoch + 1) % self.create_plots_interval == 0:
