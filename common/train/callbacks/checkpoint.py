...
class Checkpoint(Callback):
    ...
    def __init__(self, accelerator: Accelerator, output_dir: str, folder_name: str = DEFAULT_FOLDER_NAME, create_dir: bool = True,
                 save_interval: int = 1, n_saved: int = 1, score_function: Callable[[Trainer], float] = None, score_name: str = "",
                 largest: bool = True, save_as_state_dict: bool = True):
        """
    ...
    def on_fit_initialization(self, trainer):
        if self.accelerator.is_main_process and self.create_dir and not os.path.exists(self.checkpoints_dir):
    ...
    def on_epoch_end(self, trainer):
        ...
        if self.accelerator.is_main_process and (trainer.epoch + 1) % self.save_interval == 0:
