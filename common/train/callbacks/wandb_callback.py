...
class WandBCallback(Callback):
    ...
    def __init__(self, accelerator: Accelerator, project_name: str, experiment_name: str, experiment_config: dict, entity_name: str = "",
    ...
    def on_fit_start(self, trainer, num_epochs):
        if self.accelerator.is_main_process and self.track_model:
    ...
    def on_epoch_train_end(self, trainer, metric_values):
        if self.accelerator.is_main_process and self.epoch_log_interval > 0 and (trainer.epoch + 1) % self.epoch_log_interval == 0:
    ...
    def on_fit_termination(self, trainer):
        if self.accelerator.is_main_process and not self.manual_finish:
            self.finish()
